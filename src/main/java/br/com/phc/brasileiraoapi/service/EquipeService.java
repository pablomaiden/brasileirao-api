...
import br.com.phc.brasileiraoapi.exception.BadRequestionException;
...
public class EquipeService {
	...
	public Equipe inserirEquipe(EquipeDTO dto) {
		...
		if (exists) {
			throw new BadRequestionException("Já existe uma equipe cadastrada com o nome informado.");
		}
	...
	public void alterarEquipe(Long id, EquipeDTO dto) {
		...
		if (!exists) {
			throw new BadRequestionException("Não foi possivel alterar a equipe: ID inexistente");
		}
		Equipe equipe = modelMapper.map(dto, Equipe.class);
		equipe.setId(id);
